<%@ page language="java" contentType="text/html; charset=ISO-8859-1" pageEncoding="ISO-8859-1" import="java.util.ArrayList" import="java.util.List" import="dao.ColaboradorDAO" import="dao.TarefaDAO" 
import=" entity.Colaborador" import=" entity.Tarefa"%>
...
<html lang="en">
    ...
    <body translate="no">
        <div class="container">
            ...
            <div class="content">
                <div id="cadastro">
                    <form method="post" action="http://localhost:8080/DesafioWeb/alterarTarefa">
                        <h1>Editar tarefa</h1>
                        <p></p>
						<%
							...
							Tarefa tarefa = tarefaDAO.Consultar(idTarefa);
                        ...
                        <input id="nome_desc" name="nome_desc" required oninvalid="this.setCustomValidity('Campo requerido')" onchange="try{setCustomValidity('')}catch(e){}" type="text" value="<%= tarefa.getDescrTarefa() %>" />
                        <label class="col-md-2 control-label" for="dtInicio">Data início</label>
                        <input id="dataInicio" name="dataInicio" class="form-control input-md" required oninvalid="this.setCustomValidity('Campo requerido')" type="date" onChange="verificaData()" value="<%= tarefa.getDataHoraInicio() %>" />
                        <label class="col-md-2 control-label" for="dataFinal">Data Final (Não inclusa)</label>
                        <input id="dataFinal" name="dataFinal" class="form-control input-md" required oninvalid="this.setCustomValidity('Campo requerido')" type="date" onChange="verificaData()" value="<%= tarefa.getDataHoraFim() %>"/>
                        <input type="hidden" name ="idTarefa" value="<%= idTarefa %>"/>
						
                        <p>
                        	
                           <input type="submit" value="Alterar"/>
                            <a class='voltar'href="http://localhost:8080/DesafioWeb/view/alterarTarefa.jsp" role="button">Voltar</a>
                        </p>
                    </form>
                    <script>
